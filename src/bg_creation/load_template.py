...
class TemplateLoader:
    ...
    def load_template(self, node_id: str) -> Optional[Template]:
        ...
        try:
            ...
            # Check if this is the new format (with json_schema) or old format
            if template_data.get('type') == 'json_schema' and 'json_schema' in template_data:
                # New format - return the template data directly for use with OpenAI
                self.logger.info(f"Successfully loaded template for {node_id}")
                return template_data
            else:
                # Old format - validate required fields
                ...
                # Create Template object (old format)
                template = Template(
    ...
    def create_extraction_schema(self, template) -> Dict[str, Any]:
        """
        Create a schema for fact extraction based on template with tier structure.
        
        Args:
            template: Template object (old format) or dict (new format)
            
        Returns:
            Dict[str, Any]: Schema for fact extraction
        """
        # Check if this is the new format (dict with json_schema)
        if isinstance(template, dict) and template.get('type') == 'json_schema':
            # New format - return the template directly
            return template
            
        # Old format - create schema from Template object
        schema = {
